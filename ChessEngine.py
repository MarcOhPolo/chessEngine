...
class Engine:
    ...
    def evalFunct(self):
        compt = 0
        occSquares = Engine.get_all_set_bits(self.board.occupied)
        #Sums up the material values
        #optmised, 2x faster now, iterated 64 times rather than only all occupied spaces
        for i in range(len(occSquares)):
            compt+=self.squareResPoints(occSquares[i])
        compt += self.mateOpportunity() + self.openning() + 0.001*rd.random()
        return compt

    def mateOpportunity(self):
        if (self.board.legal_moves.count()==0 and (self.board.is_checkmate() == True)):
